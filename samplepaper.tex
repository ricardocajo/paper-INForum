...
\paulo{Perhaps the RW could go in here to save space, in a paragraph}
...
We have developed a compiler for the language that generates a ROS module that monitors the existing project. The module listens to relevant information from both the component \paulo{both the component? What component are we referring in here?} and the simulator, and if a violation of any property occurs, it provides a detailed error message to the user.
...
\paulo{this could perhaps be described as a equation, and then ref the equation in the previous and the next paragraph}
\vspace{3mm}

\paulo{You need to explain each part of the property. What is the first part of the after\_until, the second part of the distance being $>$ 1, and the eventually.}
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{\paulo{small "title" here: something that describes this process}:} the developer describes in the language the properties of the robotic system one wants to monitor in a \texttt{.txt} file extension.
    \item \textbf{\paulo{small "title" here: something that describes this process}:} The specified properties are compiled and a python file is generated capable of running as a ROS node.
    \item \textbf{\paulo{small "title" here: something that describes this process}:} The node can be run whenever testing the system and will listen to relevant topics~\paulo{this is correct but raises some questions. What are the relevant topics? How does it decide which ones are relevant? the reader might be a bit confused. We don't have space to go into details, so if there is a way to tell the reader in a high level way, perhaps a reword is all that is needed} and perform the computations needed to verify the specified properties.
\end{enumerate}


\section{Specification Language for Robotics Properties}

The domain-specific language relies on an adaptation of LTL to express temporal relations of and between simulation objects. The domain-specific language also has shortcuts to express the absolute values of certain useful concepts of objects in a simulation. \paulo{why do you have these shortcuts? The expectation is that these will somehow help the user, make the language more usable?}

\paulo{We need something here that links both subsections: This section presents the X, Y and Z that represent specificities of the language. Subsection 3.1 describes the process in which X through X1 and X2 is able to .... Subsection 3.2 enumerates Y, etc, etc...}

\subsection{Temporal Keywords}
\paulo{add labels to subsecs and secs}
